...
| 功能 | 说明 |
|------|------|
| 🎯 加权随机 | 三层公平性保护 — 逆频次加权 + 平均值过滤 + 候选池下限保障 |
| 👥 多人抽取 | 右键 → 多人抽取...（Ctrl+M），无放回一次抽取 2–10 人 |
| 🔁 连续不重复 | 同一学生不会连续两轮被抽中，避免"刚被点又再点" |
...
| 📊 统计弹窗 | 可排序表格，前三名金银铜高亮，含当前概率列，支持导出/重置 |
| 📈 概率分布 | 水平条形图面板，颜色从绿（高概率）渐变到红（低概率） |
...
```
├── 导入名单
├── 统计 ▸ (查看统计信息 / 查看概率分布 / 查看抽取历史 / 清除抽取历史 / 重置数据 / 导出统计数据)
├── 多人抽取...
...
- **概率算法**: 三层公平性保护 — 逆频次加权 `p = 1/(N×(count+1))` + 平均值过滤（差距 > 阈值时排除高频学生）+ 候选池下限保障（≥30%），`random.choices()` 抽取，结果缓存预热
- **多人无放回**: 循环抽取 N 次，每轮从剩余池移除已选学生，含连续不重复保护
